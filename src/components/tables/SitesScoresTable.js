...
function SitesScoresTable({ sites }) {
  ...
  const sortedSites = useMemo(() => {
    ...
    return [...sites].sort((a, b) => {
      const aValue = a.audits[0]?.auditResult.scores[sortDescriptor.column];
      const bValue = b.audits[0]?.auditResult.scores[sortDescriptor.column];
  ...
  return (
    ...
    >
      <TableHeader>
        <Column key="baseURL" defaultWidth="1fr" allowsResizing>Base URL</Column>
