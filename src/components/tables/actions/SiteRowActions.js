...
function SiteRowActions({ site, auditType, audit, updateSites }) {
  ...
  if (!audit) {
    ...
    disabledKeys.push('psi-diff');
  }

  if (!isObject(audit?.previousAuditResult)) {
    disabledKeys.push('psi-diff');
  }
  ...
  return (
    <>
      <ActionMenu onAction={handleAction} isQuiet disabledKeys={disabledKeys}>
        ...
        {audit && (
          <Section title="Latest Audit">
            <Item key="audit-details" textValue="audit details"><Search size="S"/><Text>Audit Details</Text></Item>
            <Item key="psi-report" textValue="psi report"><Globe size="S"/><Text>PSI Report</Text></Item>
            <Item key="psi-diff" textValue="psi diff"><Globe size="S"/><Text>Compare to Previous</Text></Item>
          </Section>
        )}
